...
{
    public class Zkem : IZKEM, CZKEM
    {
        public int CommPassword { get; set; }

        private readonly CZKEM objCZKEM = new CZKEM();
        private readonly Action<object, string> RaiseDeviceEvent;
        ...
        public Zkem(Action<object, string> RaiseDeviceEvent)
        {
        ...
        /// <param name="Port">default port: 4370</param>
        /// <returns></returns>
        public bool Connect_Net(string IPAdd, int Port = 4370)
        {
            SetCommPassword(CommPassword);
        ...
        {
            RaiseDeviceEvent(this, ZkemHelpers.ActionEvent());
        }

        private void zkemClient_OnAttTransactionEx(string EnrollNumber, int IsInValid,
            int AttState, int VerifyMethod, int Year, int Month, int Day, int Hour, int Minute, int Second, int WorkCode)
        {
            // All Transaction Log are all successfully valid and verified

            var o = new AttTransactionExModel
            {
                UserIdNum = int.Parse(EnrollNumber),
                VerifyMethod = VerifyMethod,
                LogDateTime = new DateTime(Year, Month, Day, Hour, Minute, Second),
            };

            RaiseDeviceEvent(o, ZkemHelpers.ActionEvent());
        }
        ...
        public bool StartIdentify() => StartIdentify();
        ...
        public bool GetVendor(ref string strVendor) => objCZKEM.GetVendor(strVendor);

        public bool GetWiegandFmt(int dwMachineNumber, ref string sWiegandFmt) => objCZKEM.GetWiegandFmt(dwMachineNumber, sWiegandFmt);

        public bool GetSDKVersion(ref string strVersion) => objCZKEM.GetSDKVersion(strVersion);

        public bool GetSerialNumber(int dwMachineNumber, out string dwSerialNumber) => objCZKEM.GetSerialNumber(dwMachineNumber, out dwSerialNumber);

        public bool GetDeviceMAC(int dwMachineNumber, ref string sMAC) => objCZKEM.GetDeviceMAC(dwMachineNumber, sMAC);
        ...
        public bool GetPlatform(int dwMachineNumber, ref string Platform) => objCZKEM.GetPlatform(dwMachineNumber, ref Platform);

        public bool GetProductCode(int dwMachineNumber, out string lpszProductCode) => objCZKEM.GetProductCode(dwMachineNumber, out lpszProductCode);
        ...
        public bool GetSysOption(int dwMachineNumber, string Option, out string Value) => objCZKEM.GetSysOption(dwMachineNumber, Option, out Value);
        ...
        public bool GetDeviceIP(int dwMachineNumber, ref string IPAddr) => objCZKEM.GetDeviceIP(dwMachineNumber, ref IPAddr);

        public bool GetDeviceStatus(int dwMachineNumber, int dwStatus, ref int dwValue) => objCZKEM.GetDeviceStatus(dwMachineNumber, dwStatus, ref dwValue);
        ...
        public int CommPort { get; set; } = 4370;
        ...
        {
            CommPassword = CommKey;
            return objCZKEM.SetCommPassword(CommPassword);
            //throw new NotImplementedException();
        }
        ...
        public bool SearchDevice(string commType, string address, out string DevBuffer, int DevBufferSize)
            => objCZKEM.SearchDevice(commType, address, out DevBuffer, DevBufferSize);
